...
<https://techzone.ibm.com/collection/liberty-getting-started-labs-demos>
...
1.  Clone the GitHub repository.
    ...
    <kbd>![git_clone.png](./images/media/git_clone.png)</kbd>
...
2.  Change to the workshop directory that contains the cloned repository artifacts. Then list the directory contents.
    ...
    <kbd>![m2m-ws-sample.png](./images/media/m2m-ws-sample.png)</kbd>
...
1.  Download the Mono2Micro-CLI.zip and unzip it in the Student directory folder:
        ...
        unzip Mono2Micro-CLI.zip -d /home/itzuser/Student/Mono2Micro-CLI
        cd /home/itzuser/Student/Mono2Micro-CLI
...
For this lab, the monolith source files tree can then be found in /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith directory.
...
1.  Build the original version of the DefaultApplication
    ...
    Change to the directory location of the application code, and run the maven build.
        ...
        cd /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith
...
3.  Run the code analyzer using the following command:
    ...
    Use the "**-a**" option to specify the application binary code of the Default Application and the option --analyze-all to force all packages inside the binary to be analyzed.
        ...
        ./mono2micro analyze -a /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith/DefaultApplication-ear/target/DefaultApplication.ear --analyze-all -i out
...
1.  Configure the Liberty server to load the Binary Instrumenter agent.
    ...
    Create a jvm.options that will be used by Liberty server
        ...
        cd /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith/DefaultApplication-ear/target

        echo '-javaagent:/home/itzuser/Student/Mono2Micro-CLI/instrumenter/minerva-agent-1.0.jar=/home/itzuser/Student/Mono2Micro-CLI/DefaultApplication-mono2micro/instrumenter-config.json' \
...
2.  Run the commands below to Start the Liberty server and check that the server is in the running state
    ...
    <p>/home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith/DefaultApplication-ear/target/liberty/wlp/usr/servers/DefaultApplicationServer</p></td>
    </tr>
    </tbody>
    </table>
...
2.  View the Liberty log file to ensure the log contains the trace
    statements from the application.
        ...
        cat /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith/DefaultApplication-ear/target/liberty/wlp/usr/servers/DefaultApplicationServer/logs/messages.log | grep '\<Entering\>\|Exiting'
...
The **/home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/** directory contains the subdirectories within which the data files are placed:
 ...
 **analyzer**
  ...
  - **config.ini** Optional file to configure various parameters for the analysis tool. If one doesn’t exist, AI engine generates one for you with default values.
        ...
        ls -R /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data
...
3.  Run the AI engine using the following command:
        ...
        ./mono2micro recommend -d /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data
...
4.  **REFERENCE ONLY\!** Listed here are some of the notable files that
    ...
    - **Cardinal-Report**.html is a detailed report of all the application partitions, their member classes, outward facing classes, etc
          ...
          /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/mono2micro/mono2micro-output/Cardinal-Report.html
 
    - **Oriole-Report.html** is a summary report of all the application
 partitions and their associated business use cases
 
          /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/mono2micro/mono2micro-output/Oriole-Report.html
    ...
    - **final_graph.json** is the full set of application partition  recommendations (natural seams and business logic) and associated  details, viewable in the Mono2Micro UI
          ...
          /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/mono2micro/mono2micro-output/oriole/final_graph.json
    ...
    - **cardinal/\*** is a folder that contains a complete set of input files (based on the partitioning) for the next and last stage of the Mono2Micro pipeline, running the code generator
          ...
          /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/mono2micro/mono2micro-output/cardinal/*
...
4.  Load the **final\_graph.json** file in the mono2micro UI
    ...
    > home \> itzuser > Student \> m2m-ws-sample \> defaultapplication \>  mono2micro-analysis \> oriole \> final\_graph.json
...
6.  Click on the “**Save partitions**” button to save the updated custom view. Accept the defaults. The customized **final\_graph.json** file is saved to the **“/home/itzuser/Downloads”** folder.
...
1.  Copy the customized **final_graph.json** file from users “Downloads” folder to a known location by Mono2Micro’s AI
    ...
    a.  Run the following commands to copy the file, change to the target directory, and list the files and ensure the **`custom_graph.json`** has been copied to the desired directory
        ...
        cp /home/itzuser/Downloads/final_graph.json /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/custom_graph.json

        cd /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data
...
3.  Edit the **config.ini** file to reference the new **custom_graph.json** file to be used for regenerating the partition recommendations. (Use any editor available)
    ...
    If you messed it up, replace the content of the file with this:
...
4.  Go to the Mono2Micro-CLI directory and then rerun the AI engine with the “**refine”** command to generate the partitioning
    recommendations based on the updated graph file.
        ...
        ./mono2micro refine -d /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data
...
1.  The AI engine created a new folder based on the user modified graph in the following directory:
    ...
    > */home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/mono2micro/mono2micro-user-modified*

    a.  List the files / folders of the generated directory

        ls -l /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/mono2micro/mono2micro-user-modified
...
2.  View the generated Cardinal report to verify the partitions and
    exposed services are defined as expected.
        ...
        cd /home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/mono2micro/mono2micro-user-modified
...
The code generator requires the following input artifacts and is referenced in the code generator command for proper execution:
  ...
  - The parent folder of the **original DefaultApplication monolith** application.
    ...
    > /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith
  ...
  - The **cardinal** folder from the mono2micro-user-modified directory that was generated by the AI engine using the “**refine”** command.
    ...
    > /home/itzuser/Student/m2m-ws-sample/defaultapplication/mono2micro-analysis-custom/cardinal
...
<p>/home/itzuser/Student/m2m-ws-sample/defaultapplication/<strong>mono2micro-analysis-custom/cardinal</strong></p>
<p>If you would rather use the cardinal folder that was generated during the lab, use:</p>
<p>/home/itzuser/Student/m2m-ws-sample/defaultapplication/application-data/mono2micro/<strong>mono2micro-user-modified/cardinal</strong></p></td>
</tr>
</tbody>
</table>
...
2.  Run the code generator using the following command:
        ...
        ./mono2micro transform -p /home/itzuser/Student/m2m-ws-sample/defaultapplication/mono2micro-analysis-custom/cardinal -s /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith
...
1.  Examine the **CardinalFileSummary.txt** file.
    ...
    > /home/itzuser/Student/m2m-ws-sample/defaultapplication/mono2micro-analysis-custom/cardinal/cardinal-codegen

    a.  Open the CardinalFileSummary.txt file using an available editor

        cd /home/itzuser/Student/m2m-ws-sample/defaultapplication/mono2micro-analysis-custom/cardinal/cardinal-codegen
...
> /home/itzuser/Student/m2m-ws-sample/defaultapplication
...
1.  Review the **moveResourcesToPartitions.sh** shell script.
        ...
        cd /home/itzuser/Student/m2m-ws-sample/defaultapplication/scripts
    ...
    a.  The script defines a bunch of variables referencing various directories for copying files.
        Make dure that the workdir is set to /home/itzuser/Student/m2m-ws-sample. If not adjust the settings and save the file.
...
4.  Use a graphical **File Explorer** or **Terminal** window to see the non-Java files now in each of the partitions, and in the same directory structure as the original monolith.
    ...
    - /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith-web

    - /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith-partition0
...
1.  Use the **refactorPartitions.sh** shell script to perform the partition refactoring
    ...
    a. Open the script and make sure that the workdir is set to **/home/itzuser/Student/m2m-ws-sample**. If not adjust the settings and save the file.
...
1.  Compile the **monolith-web** microservice via command line
    ...
    a.  Change to the **monolith-web** directory, which contains the top-level pom.xml for building the monolith-web microservice
        ...
        cd /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith-web
...
2.  Compile the **monolith-partition0** microservice via command line
    ...
    a.   Change to the **monolith-partition0** directory, which contains the top-level pom.xml for building the monolith-web microservice
        ...
        cd /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith-partition0
...
1.  Navigate to the new “Utilities” **pom.xml** file to view its contents in the web partition. This file was created in ALL partitions.
        ...
        gedit /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith-web/application/pom.xml
...
1.  Navigate to the top-level **pom.xml** file in the **web partition** to view this update. This update was made to all the top-level
    pom.xml in each partition.
        ...
        gedit /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith-web/pom.xml
...
1.  Navigate to the **JAXRSConfiguration** java file in the **partition0** **partition** to view this update.
        ...
        gedit /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith-partition0/DefaultWebApplication/src/main/java/com/ibm/defaultapplication/JAXRSConfiguration.java
...
1.  Navigate to the server.xml file in the **web** **partition** to view this update.
        ...
        gedit /home/itzuser/Student/m2m-ws-sample/defaultapplication/monolith-web/DefaultApplication-ear/src/main/liberty/config/server.xml
...
There were only 3 changes that we had to do, in addition to the obviously issuing commands using my own path instead of /home/itzuser/Student:
